#!/usr/bin/env bash
# scripts/test-leave-space.sh
# Runs supabase/tests/leave_space.sql (Task 4) against a freshly-reset local
# database. The file proves the leave-a-household routine on a fixture and
# manages its own ON_ERROR_STOP / assertion discipline internally, so this
# runner just resets and invokes it.
set -euo pipefail
...
psql "$DB_URL" -f supabase/tests/leave_space.sql
echo "leave_space tests passed"
